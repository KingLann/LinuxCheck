...
echo ""
echo " ========================================================= "
echo " \        Linux应急处置/信息搜集/漏洞检测脚本 V3.1      / "
echo " ========================================================= "
echo " # 支持Centos、Debian系统检测                    "
echo " # author：zhao                    "
echo " # https://github.com/KingLann/LinuxCheck                    "
echo " # 更新日期：2024年9月13日                    "
echo " # 参考来源：                "
echo " #   1.LinuxCheck https://github.com/al0ne/LinuxCheck  "
echo -e "\n"
...
process_check() {
  ...
  # 使用 netstat 命令列出所有进程
  output=$(netstat -anplt 2>/dev/null)
  # 检查是否存在 su 相关进程
  if echo "$output" | grep -q 'su'; then
      print_msg "疑似存在SSH软链接后门"
      print_msg "相关进程信息:"
      print_code "$(echo "$output" | grep 'su')"
  else
      print_msg "未检测到与 'su' 相关的可疑进程。"
  fi
}
...
crontab_check() {
  ...
  print_msg "crontab -l"
  print_code "$(crontab -u root -l | grep -a -v '#')"
...
ssh_check() {
  ...
  #SSHD
  # 检查 /usr/sbin/sshd 文件的类型
  file_type=$(file /usr/sbin/sshd)

  print_msg "### SSHD"
  print_msg "/usr/sbin/sshd文件时间："
  print_code "$(stat /usr/sbin/sshd | grep -P 'Access|Modify|Change')"
  # 判断文件类型是否为 ELF 可执行文件
  if echo "$file_type" | grep -q 'ELF'; then
      print_msg "/usr/sbin/sshd 文件是 ELF 可执行文件，正常。"
  else
      print_msg "文件疑似被篡改：/usr/sbin/sshd 不是 ELF 可执行文件！"
      print_code "$file_type"
  fi

  # SSH Wapper 后门检查
  print_msg "### SSH Wapper 后门检测 "
  # 判断文件类型是否为文本
  if echo "$file_type" | grep -q 'text'; then
      # 如果是文本类型，检查文件中是否存在关键字 /bin/sh 和 getpeername(STDIN)
      if grep -q '/bin/sh' /usr/sbin/sshd && grep -q 'getpeername(STDIN)' /usr/sbin/sshd; then
          print_msg "存在SSH Wapper 后门"
          print_code "$(cat /usr/sbin/sshd)"
      else
          echo "/usr/sbin/sshd 文件中未检测到后门相关的关键字。"
      fi
  else
      echo "/usr/sbin/sshd 文件不是文本类型，无法检查关键字。"
  fi
...
collect_container_info() {
    ...
    for container_id in $container_ids; do
        ...
        container_name=${container_name#/}  # 移除容器名前面的斜杠
        ...
        container_id_short=$(echo $container_id | cut -c 1-12)

        # 获取容器 IP
        local container_ip
        container_ip=$(docker inspect --format '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $container_id)

        # 检查容器是否有 Privileged 权限
        local privileged
        privileged=$(docker inspect --format '{{.HostConfig.Privileged}}' $container_id)
        local privileged_status="No"
        [ "$privileged" == "true" ] && privileged_status="Yes"

        # 获取容器创建时间
        local create_time
        create_time=$(docker inspect --format '{{.Created}}' $container_id)

        # 获取容器挂载的卷信息
        local mounted_volumes
        mounted_volumes=$(docker inspect --format '{{range .Mounts}}{{.Source}}:{{.Destination}} {{end}}' $container_id)

        # 获取容器的端口映射信息
        local port_mappings
        port_mappings=$(docker inspect --format '{{range $p, $conf := .NetworkSettings.Ports}}{{if $conf}}{{$p}} -> {{(index $conf 0).HostPort}},{{end}}{{end}}' $container_id)
        port_mappings=${port_mappings%,}  # 移除最后一个逗号

        # 将当前容器的信息追加到变量中
        local container_info
        container_info=$(cat <<EOF

容器ID: $container_id_short
容器名: $container_name
容器IP: $container_ip
容器镜像: $container_image
特权模式: $privileged_status
创建时间: $create_time
卷挂载(宿主机/容器): $mounted_volumes
端口映射(容器/宿主机): ${port_mappings:-无}
---------------------------------------
EOF
)

        # 将当前容器信息追加到所有容器信息变量中
        all_container_info+="$container_info"
    done

    # 输出所有容器的信息
    print_code "$all_container_info"
}
...
docker_check() {
  ...
  print_code "$(docker ps -a)"
  print_msg "Docker运行的镜像信息，请注意开启特权模式的容器；"
  # 调用函数
  collect_container_info
